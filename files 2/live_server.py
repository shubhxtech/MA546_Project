...
from news_trading_pipeline import NewsTradingPipeline, CONFIG
...
SIMULATION_PARAMS = {
    ...
    "ml_params": {
         ...
         "oos_window": 1,
         "oos_window_unit": "months",
         "top_m": 10,
         "opt_protocol": "SHAP Weighting",
         "models": ["Linear", "LASSO", "Ridge", "RF"]
    },
    "next_rebalance_date": pd.Timestamp("2070-01-01"),
...
def init_global_caches():
    global GLOBAL_HEADLINES, GLOBAL_DATES, PIPELINE_INSTANCE
...
def execute_ml_rebalance(virtual_time):
    ...
    try:
        ...
        is_ret = (raw_price / raw_price.shift(1) - 1).fillna(0.0)
    except Exception as e:
        push_exec_log(f"Pricing data error: {e}")
        return None
    # Prepare features — only tickers that appear in both price and sentiment
    valid_tickers = [t for t in top_m_tickers if t in is_ret.columns and t in sent_df.columns]
    ...
    panel_X_rows = []
    panel_y_vals = []
    ret_aligned = is_ret[valid_tickers].reindex(raw_features.index)
    ...
    if len(panel_X_rows) < max(10, len(valid_tickers)):
        push_exec_log(f"Insufficient panel rows ({len(panel_X_rows)}) — need ≥{len(valid_tickers)}")
        return None
    ...
    if p["opt_protocol"] == "SHAP Weighting":
        ...
        cov = is_ret[valid_tickers].cov()
        weights = optimize_portfolio(
            pred_ret, cov,
            framework=p["opt_protocol"],
            returns_hist=is_ret[valid_tickers]
        )
...
def simulation_worker():
    while True:
        ...
        try:
            start_t = time.time()
            art = PIPELINE_INSTANCE.ingest(headline, str(virtual_time))
            
            is_valid_signal = False
            if art is not None:
                art = PIPELINE_INSTANCE.extract_entities(art)
                if PIPELINE_INSTANCE.filter_relevance(art):
                    art = PIPELINE_INSTANCE.analyze_sentiment(art)
            ...
            with state_lock:
                ...
                if is_valid_signal:
                    ...
                    for t in art.tickers:
                        if t.startswith("__"): # Skip NLP-only indices (NIFTY, SENSEX, etc.)
                            continue
                        t_ns = t + ".NS"
        ...
        time.sleep(0.0001)  # Absolute raw iteration boundaries
...
def compute_tearsheet():
    """Compute full performance tear sheet from accumulated weight history."""
    wh = SIMULATION_PARAMS.get("daily_weights_history", {})
    if not wh:
        return {}
        ...
        if url.path == "/api/start-sim":
            ...
            try:
                ...
                with state_lock:
                    ...
                    APP_STATE["metrics"] = {"articles_processed": 0, "signals_generated": 0, "nli_filtered_out": 0, "final_pnl": None, "live_pnl": "0.00%"}
                    APP_STATE["tearsheet"] = {}
                    ...
                    # First rebalance fires 3 days in — ensures portfolio visible quickly
                    SIMULATION_PARAMS["next_rebalance_date"] = s_dt + pd.DateOffset(days=3)
...
def start_server():
    init_global_caches()
    server = HTTPServer((HOST, PORT), LiveAPIHandler)
